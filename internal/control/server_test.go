...
type mockQueryRunner struct {
	execFn func(ctx context.Context, tenantID, query string) (int64, error)
}

func (m *mockQueryRunner) ExecForTenant(ctx context.Context, tenantID, query string) (int64, error) {
	if m.execFn != nil {
		return m.execFn(ctx, tenantID, query)
	}
	return 0, nil
}
...
func newTestServer(t *testing.T, cat control.CatalogStore, prov control.TenantProvisioner) http.Handler {
	...
	return control.NewServer(prov, cat, authSvc, "admin-key", &mockQueryRunner{})
}
